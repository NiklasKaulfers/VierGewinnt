...
public class GameTest {
    ...
    void testPlaceStone1(){
        ...
        assertFalse(board.getIsFull(), "Falscher IsFull Status");
    }
    @Test
    ...
    void testFullBoardWithoutWinner() {
        ...
        assertEquals(0, board.getWhoHasWon(), "Falscher Gewinner Status");
        assertTrue(board.getIsFull(), "Falscher IsFull Status");
    }
    @Test
    ...
    void gameprint(TileInterface[][] t){
        StringBuilder s = new StringBuilder("Spielfeld: \n");
        for(int i = 0; i < 6; i++){
            for(int j = 0; j < 7; j++){
                s.append("| ").append(t[i][j].getStatus()).append(" |");
            }
            s.append("\n");
        }
