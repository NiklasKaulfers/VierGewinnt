...
import org.hszg.api.BoardInterface;
...
public class Board implements BoardInterface, BoardTestInterface {
    ...
    private Result miniMax(Board board, int depth, int alpha, int beta, int maximizingPlayer) {
        ...
        if (maximizingPlayer == (board.isPlayer1sTurn ? 1 : 2)) {
            ...
            for (Integer possibleMove : possibleMoves) {
                ...
                newBoard.placeStone(possibleMove);
    ...
    public boolean loadPvP() {
        ...
        boolean ret = false;
        try {
            fileInputStream = new FileInputStream("PvP.data");
    ...
    private String createSavecode() {
        ...
        output.append(seperator);        //seperator einfügen
    ...
    private void setValuesFromSavecode(String savecode) {
        ...
        char[] savecodeCharArray = savecode.toCharArray();    //string zu array von chars
        // todo: what are each of those variables?
        int countA = 0;
        String v = "";
        ...
        for (char ch : savecodeCharArray) {
            ...
            if (!checkB) {
                ...
                field[r][c] = new Tile(Character.getNumericValue(ch));
    ...
    private String getSavecodeFromFile() {
        ...
        try {
            while ((ch = fileReader.read()) != -1) output.append((char) ch);
